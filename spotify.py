...
def on_message(message, data):
    ...
    try:
        if message['payload'] is not None:
            print(bcolors.OKGREEN + "[+] Found pattern for CmdAddText! Address: " + bcolors.ENDC + bcolors.BOLD +
                  message['payload'] + bcolors.ENDC)
            CMDADDTEXT = message['payload']

            cmdaddtextscript = session.create_script("""
            Interceptor.attach(ptr("%s"), {
                onEnter(args) {
                    if (ptr(args[6]).readCString().indexOf("Prefetch") == -1 && ptr(args[6]).readCString().indexOf("track") !== -1) {
            ...
            def on_message(message, data):
                ...
                try:
                    if message['payload'] is not None and "spotify:track" in message['payload']:
                        print(bcolors.OKGREEN + "[+] Listening to track: " + bcolors.ENDC + message['payload'])
                        endpoint = "https://api.spotify.com/v1/tracks/" + message['payload'][-22:]
                        headers = {"Authorization": "Bearer " + {True: get_access_token(), False: token}[token == ""]}
                        response = requests.get(endpoint, headers=headers).json()
                        endpoint = "http://localhost:8080/history/insert"
                        dopost = requests.post(endpoint, json=response)
                        print(bcolors.OKCYAN + bcolors.BOLD + ', '.join([str(x['name']) for x in response['artists']]) + " - " + response[
                            'name'] + bcolors.ENDC)
                        print(bcolors.OKCYAN + bcolors.BOLD + response['album']['name'] + " - " + str(response['track_number']) + "/" + str(response['album']['total_tracks']) + bcolors.ENDC)
                        total_seconds = int(response['duration_ms'] / 1000)
                        minutes = int(total_seconds / 60)
                        seconds = int(total_seconds - minutes * 60)
                        print(bcolors.OKCYAN + bcolors.BOLD + "{}:{:02}".format(minutes, seconds))
    ...
    function processNext(){
		...
		Memory.scan(process.base, process.size, '%s', {
			...
			onError: function(reason){
					console.log("[!] Access violation while scanning memory.");
				}, 
			onComplete: function(){
                    processNext();
				}
