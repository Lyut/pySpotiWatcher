...
def on_message(message, data):
    ...
    try:
        if message['payload'] is not None and "spotify:track" in message['payload']:
            ...
            output = subprocess.getoutput("curl \"" + response['album']['images'][0]['url'] + "\" -o cover.png")
...
#with open("/opt/spotify/patches.js", "r") as file:
#    devmodescript = session.create_script(file.read())
#
#devmodescript.on('message', on_message)
#devmodescript.on("destroyed", on_destroyed)
#devmodescript.load()


with open("/opt/spotify/patches.js", "r") as file:
    cefparseurlscript = session.create_script(file.read())
